...
@section('content')
    <h2 class="h4 mb-1">Mouvements de trésorerie</h2>
    ...
    <form method="GET" action="{{ route('rapports.tresorerie') }}" class="row g-2 mb-3 d-print-none">
        ...
        @if (request()->hasAny(['debut', 'fin', 'compte_tresorerie_id', 'type']))
            <div class="col-auto">
                <x-bouton-reinitialiser :route="route('rapports.tresorerie')" />
            </div>
        @endif
        <div class="col-auto ms-auto">
            <x-export-buttons :pdf-route="route('rapports.tresorerie.pdf', request()->query())" :excel-route="route('rapports.tresorerie.excel', request()->query())" />
        </div>
    </form>
