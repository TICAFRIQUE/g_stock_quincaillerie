...
@section('content')
    <h2 class="h4 mb-1">Rapport des ventes</h2>
    @php
    ...
    <form method="GET" action="{{ route('rapports.ventes') }}" class="row g-2 mb-3 d-print-none">
        ...
        @if (request()->hasAny(['debut', 'fin', 'magasin_id', 'caissier_id', 'caisse_id']))
            <div class="col-auto">
                <x-bouton-reinitialiser :route="route('rapports.ventes')" />
            </div>
        @endif
        <div class="col-auto ms-auto">
            <x-export-buttons :pdf-route="route('rapports.ventes.pdf', request()->query())" :excel-route="route('rapports.ventes.excel', request()->query())" />
        </div>
    </form>
